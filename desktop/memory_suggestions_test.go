...
func TestMemorySuggestionsForTabUsesSelectedTab(t *testing.T) {
	...
	wantSkillPath := filepath.Join(selectedCwd, ".qiaotongagent", "skills", "selected-workflow", "SKILL.md")
	if skillPath != wantSkillPath {
		t.Fatalf("skill path = %q, want %q", skillPath, wantSkillPath)
	}
	if _, err := os.Stat(filepath.Join(activeCwd, ".qiaotongagent", "skills", "selected-workflow", "SKILL.md")); !os.IsNotExist(err) {
...
func TestMemorySuggestionsAcceptSkillCandidate(t *testing.T) {
	...
	for _, item := range view.Skills {
		if item.Name == "qiaotongagent-pr-followup" {
			candidate = item
			break
		}
	}
	if candidate.Name == "" {
		t.Fatalf("MemorySuggestions() skills = %+v, want qiaotongagent-pr-followup", view.Skills)
	}
	path, err := app.AcceptSkillSuggestion(candidate)
	if err != nil {
		t.Fatalf("AcceptSkillSuggestion: %v", err)
	}
	wantSuffix := filepath.Join(".qiaotongagent", "skills", "qiaotongagent-pr-followup", "SKILL.md")
	...
	if !strings.Contains(string(body), "Review or update a QiaotongAgent GitHub PR") {
