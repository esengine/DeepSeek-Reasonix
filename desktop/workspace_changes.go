...
func workspaceGitPathIsUntracked(base string, path string) bool {
	cmd := workspaceGit("-C", base, "ls-files", "--others", "--exclude-standard", "--", path)
	raw, err := cmd.Output()
	if err != nil {
		return false
	}
...
func (a *App) WorkspaceGitUnstage(path string) error {
	...
	return workspaceGitUnstagePath(base, rel)
}
...
func workspaceGitLocalBranchExists(base string, branch string) (bool, error) {
	cmd := workspaceGit("-C", base, "show-ref", "--verify", "--quiet", "refs/heads/"+branch)
	err := cmd.Run()
	if err == nil {
		return true, nil
	}
...
// workspaceGitUnstagePath unstages a single path.  It prefers git reset HEAD,
// which fails on an unborn HEAD (repo with no commits).  In that case it falls
// back to git rm --cached so newly-added files can still be unstaged.
func workspaceGitUnstagePath(base, path string) error {
	if err := workspaceRunGit(base, "reset", "HEAD", "--", path); err != nil {
		return workspaceRunGit(base, "rm", "--cached", "--", path)
	}
	return nil
}

func workspaceRunGit(base string, args ...string) error {
	cmd := workspaceGit(append([]string{"-C", base}, args...)...)
