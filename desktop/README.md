# QiaotongAgent Desktop (Wails shell)

A native desktop window around the QiaotongAgent Go kernel. The same
...
`desktop/` is its own Go module (`module qiaotongagent/desktop`, `replace qiaotongagent =>
...
`qiaotongagent/` so it can still import the `qiaotongagent/internal/*` kernel.
...
```sh
cd desktop
wails build          # → build/bin/QiaotongAgent(.app/.exe)
```
...
- **macOS** — open `QiaotongAgent-darwin-universal.dmg` and drag QiaotongAgent into
  ...
  ```sh
  xattr -dr com.apple.quarantine /Applications/QiaotongAgent.app
  ```
...
```sh
minisign -Vm QiaotongAgent-darwin-arm64.zip \
...
- **Windows / WebView2** — `Theme: SystemDefault` follows the OS light/dark
  ...
  `QIAOTONGAGENT_DESKTOP_DISABLE_WEBVIEW2_GPU=1` or `0` to force the fallback on or
  off.
...
The desktop app sends one anonymous ping per launch to `crash.qiaotongagent.io`:
