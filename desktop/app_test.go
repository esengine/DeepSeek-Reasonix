...
func TestSettingsUsesUserDesktopPreferencesNotProjectConfig(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(project, "qiaotongagent.toml"), []byte(`
[desktop]
language = "zh"
...
func TestDeleteProviderMigratesConfigAndOpenTabs(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("QIAOTONGAGENT_TEST_KEY", "sk-test")

	cfg := config.Default()
	cfg.DefaultModel = "prov-a/model-a2"
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", Models: []string{"model-a1", "model-a2"}, APIKeyEnv: "QIAOTONGAGENT_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "QIAOTONGAGENT_TEST_KEY"},
	}
...
func TestDeleteProviderRejectsRunningAffectedTab(t *testing.T) {
	...
	cfg.Providers = []config.ProviderEntry{
		{Name: "prov-a", Kind: "openai", BaseURL: "https://a.example.com", Model: "model-a1", APIKeyEnv: "QIAOTONGAGENT_TEST_KEY"},
		{Name: "prov-b", Kind: "openai", BaseURL: "https://b.example.com", Model: "model-b1", APIKeyEnv: "QIAOTONGAGENT_TEST_KEY"},
	}
	if err := cfg.SaveTo(config.UserConfigPath()); err != nil {
		t.Fatalf("save config: %v", err)
...
func TestSubmitToTabHistoryDisplaysRawInputAfterMemoryCompose(t *testing.T) {
	...
	ctrl.QueueMemory(`Saved memory "qiaotongagent-contributions": contribution count updated`)
...
func TestForkCreatesActiveTabWithoutSwitchingSourceController(t *testing.T) {
	...
	if err := os.WriteFile(filepath.Join(workspace, "qiaotongagent.toml"), []byte("[codegraph]\nenabled = false\n"), 0o644); err != nil {
...
func TestCapabilitiesShowsDefaultMCPAsInitializingNotDisabled(t *testing.T) {
	...
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "qiaotongagent.toml"), []byte(`
[codegraph]
enabled = false
...
`), 0o644); err != nil {
	...
	projectCfg := config.LoadForEdit(filepath.Join(dir, "qiaotongagent.toml"))
...
`), 0o644); err != nil {
	...
	if err := app.UpdateMCPServer("broken", MCPServerInput{
		Name:      "broken",
		Transport: "stdio",
		Command:   "qiaotongagent-missing-mcp-binary",
	}); err != nil {
	...
	if got := cfg.Plugins[0].Command; got != "qiaotongagent-missing-mcp-binary" {
	...
	for _, s := range view.Servers {
		if s.Name == "broken" {
			...
			if s.Command != "qiaotongagent-missing-mcp-binary" || s.Tier != "background" {
...
func TestSetMCPServerTierEnablesCodegraphAndIgnoresLegacyTier(t *testing.T) {
	...
	t.Setenv("QIAOTONGAGENT_CACHE_DIR", robustTempDir(t)) // isolate the codegraph bundle cache so Resolve fails deterministically
	dir := robustTempDir(t)
	t.Chdir(dir)
	if err := os.WriteFile(filepath.Join(dir, "qiaotongagent.toml"), []byte(`
[codegraph]
...
func TestBuiltInMCPBackgroundNotifyDoesNotDownload(t *testing.T) {
	isolateDesktopUserDirs(t)
	t.Setenv("QIAOTONGAGENT_CACHE_DIR", robustTempDir(t))
