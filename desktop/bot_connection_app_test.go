...
func TestDiagnoseBotConnectionBuildsReportDetailForMissingSecret(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/Users/alice/work/qiaotongagent",
...
func TestRememberBotConnectionRemoteStoresStableScope(t *testing.T) {
	...
	if _, err := app.upsertBotConnection(config.BotConnectionConfig{
		...
		Enabled:       true,
		Status:        "connected",
		WorkspaceRoot: "/tmp/qiaotongagent-project",
	}, nil); err != nil {
	...
	if projectMapping.Scope != "project" || projectMapping.WorkspaceRoot != "/tmp/qiaotongagent-project" || projectMapping.RemoteID != "wxid_project" {
