 ...
 * handle: the generation-aware TranscriptScrollWriter gateway.
...
// Only the generation-aware gateway may write to the transcript Virtuoso
// handle; every controller routes through it.
...
// - components/RemoteConnectWizard.tsx: the connection log's own scroller.
...
for (const file of sourceFiles(SOURCE_ROOT)) {
  ...
  lines.forEach((line, index) => {
    const virtuosoWrite = VIRTUOSO_SCROLL_RE.test(line);
    const rawWrite = RAW_SCROLLTOP_WRITE_RE.test(line) && !ALLOWED_RAW_SCROLLTOP.has(relative);
    if (!virtuosoWrite && !rawWrite) return;
    failures += 1;
    console.error(
      `check-single-scroll-writer: ${relative}:${index + 1} issues an imperative ${virtuosoWrite ? "Virtuoso" : "raw scrollTop"} write outside the allowed writer modules.\n` +
      `  ${line.trim()}\n` +
