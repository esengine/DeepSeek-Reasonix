...
export function WorkspacePanel({
  ...
  }, [expandedCommit, open, selectedPath]);
  ...
  // When cwd changes while in changed mode, reload workspace changes.
  const prevCwdForChangesRef = useRef(cwd);
  useEffect(() => {
    if (!open || viewMode !== "changed") return;
    if (cwd !== prevCwdForChangesRef.current) {
      prevCwdForChangesRef.current = cwd;
      void loadWorkspaceChanges();
    }
  }, [cwd, loadWorkspaceChanges, open, viewMode]);
  ...
  useEffect(() => {
    ...
    setTreeMenu(null);
    void loadWorkspaceChanges();
  }, [changeListRequest, loadWorkspaceChanges, open, scopedChangeRows, viewMode]);
  ...
  const sessionChanges = useMemo(
    () => allChangedFiles.filter((c) => c.sources.includes("session")),
    [allChangedFiles],
  );
  ...
  const actualTreeVisible = treeVisible && !compactTreeRail;
  const showTreeRail = previewVisible && (!actualTreeVisible || compactTreeRail);
  ...
  const renderChangeFileRows = (changes: WorkspaceChangeView[], kind: SelectedChangeKind): ReactElement[] =>
    changes.map((change) => {
      ...
      <section className="workspace-files">
        {showFilesTools && (
          <div className={`workspace-files__tools${embeddedDockMode ? " workspace-files__tools--embedded" : ""}`}>
            {changedMode && (
              <div className="workspace-files__tools-group">
                {previewVisible && (
