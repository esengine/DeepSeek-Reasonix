...
const writes: string[] = [];
const experienceWrites: string[] = [];
installDesktopHostStub({
  SetSessionExperience: async (mode: string) => { experienceWrites.push(mode); },
...
try {
  for (const mode of ["standard", "deep", "concise"] as const) {
    ...
    const experience = groups.find(group => group.getAttribute("aria-label") === t("settings.sessionExperience"))!;
    assert.equal(experience.querySelectorAll("[role=radio]").length, 3, "session experience exposes standard/deep/concise");
    const selected = [...experience.querySelectorAll<HTMLButtonElement>("[role=radio]")]
      .find(button => button.getAttribute("aria-checked") === "true")!;
    assert.equal(selected.textContent, t(`settings.sessionExperience.${mode}`), "snapshot mode is selected");
  ...
  assert.deepEqual(writes, ["read-only", "read-only", "read-only"], "permission settings send the selected preset to the host");
  assert.deepEqual(experienceWrites, [], "opening settings never rewrites the persisted experience");

  await act(async () => root.render(<LocaleProvider><SessionExperienceSettings snapshot={{ sessionExperience: "standard", defaultToolApprovalMode: "workspace-write" } as SettingsView} busy={false} apply={apply} /></LocaleProvider>));
  const experience = [...document.querySelectorAll<HTMLElement>("[role=radiogroup]")]
    .find(group => group.getAttribute("aria-label") === t("settings.sessionExperience"))!;
  await act(async () => experience.querySelectorAll<HTMLButtonElement>("button")[2].click());
  assert.deepEqual(experienceWrites, ["concise"], "selecting Concise persists the third session experience");
  ...
  assert.deepEqual(writes, ["read-only", "read-only", "read-only"], "selecting a Full access default does not persist before confirmation");
  ...
  assert.deepEqual(writes, ["read-only", "read-only", "read-only", "danger-full-access"], "confirmed Full access becomes the new-session default");
  ...
  console.log("settings: three-state session experience control, preserved approval and busy states passed");
