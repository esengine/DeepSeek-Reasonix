...

{
    ...
    questions: [
      {
        id: "decision",
        header: "修改方案",
        prompt: "baoguanPutArchive 需要对齐到什么程度？有两个方案：完整对齐归档逻辑，还是只修复当前编译错误？",
        options: [
          { label: "方案A：完全对齐", description: "完整复用归档逻辑，代码结构更一致。" },
          { label: "方案B：关键逻辑对齐", description: "只对齐关键路径，改动范围更小。" },
        ],
      },
    ],
  };
  ...
  const shelf = document.querySelector(".prompt-shelf") as HTMLElement | null;
  const meta = document.querySelector(".prompt-shelf__meta") as HTMLElement | null;
  if (!shelf || !meta) throw new Error("ask prompt shelf did not render");

  ok(shelf.classList.contains("prompt-shelf--meta-wrap"), "ask shelf opts into wrapped prompt metadata");
