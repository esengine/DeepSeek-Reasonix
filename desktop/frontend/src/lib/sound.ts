 ...
 * 通知音效系统
 *
 * 支持合成音效（默认）和 WAV 文件播放两种模式。
 * 两个场景的偏好分别存入 localStorage:
 *   notificationSoundSuccess  —— 生成完成
 *   notificationSoundAttention —— AI 提问
 *   值："synth" | "positive" | "correct" | "start" | "back"
 */

export type SoundWavPref = "synth" | "positive" | "correct" | "start" | "back";
...
function readPref(key: string): SoundWavPref {
  if (typeof localStorage === "undefined") return "synth";
  const val = localStorage.getItem(key);
  if (val === "positive" || val === "correct" || val === "start" || val === "back") return val;
  return "synth";
}
...
async function playWav(pref: SoundWavPref, volume: number): Promise<void> {
