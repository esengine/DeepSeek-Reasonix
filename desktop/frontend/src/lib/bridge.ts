...
export interface AppBindings extends SessionCatalogBindings, ProjectTreeOrganizationBindings, HistoryCatalogBindings, TaskCatalogBindings, BlankProjectBindings, QualityFloorBindings, SessionTitleBindings {
  ...
  SetCacheContext(value: string): Promise<void>;
...
function bridgeBreadcrumb(method: string): string {
  ...
  if (/^(SetModel|SetEffort|SetDefaultModel|SetPlannerModel|SetSubagentModel|SetSubagentEffort|SetMaxSubagentDepth|SetMaxSubagentConcurrency|SetMaxParallelWriters)/.test(method))
    return `model ${method}`;
  if (/^(SetDesktop|SetCloseBehavior|SetDisplayMode|SetStatusBar|SetReasoningDisplayMode|SetExpandThinking|SetAutoPlan|SetDefaultToolApprovalMode|SetCompactRatio|SetReasoningLanguage|SetCacheContext)/.test(method))
...
function makeMockApp(): AppBindings {
  ...
  const settings: SettingsView = {
    ...
    cacheContext: "mock-project",
    cacheContextProject: true,
  ...
  return {
    ...
    async SetCacheContext(_value: string) {
      // mock bridge: cache context writes to the project config; no-op here.
    },
    // ── Heartbeat mock ──
