const attachmentRefRe = /@(\.qiaotongagent\/attachments\/[^\s]+)/g;
...
function isDisplayReference(path: string): boolean {
  if (path.startsWith(".qiaotongagent/attachments/")) return true;
...
function displayAttachment(path: string, name: string): DisplayAttachment {
  if (path.startsWith(".qiaotongagent/attachments/")) {
