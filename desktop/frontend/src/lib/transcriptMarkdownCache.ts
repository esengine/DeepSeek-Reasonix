...
export class TranscriptMarkdownCache {
   ...
   * revision identifies the entry by itself. Keying on a row id as well only
   * loses hits: a row's id is a function of which host is rendering it
   * (`p3`/`c1`/`e12` while live, `he:<entryId>` or `m:<messageId>` once it is
   * history), so every transition between hosts changed the key and discarded
   * the parse. Identical text therefore shares one entry, which is correct —
   * identical text parses identically — and the stored `source` is still
   * compared on read as the collision backstop (#10143).
   */
