...
// by substring (file names embed the key, e.g. QiaotongAgent-darwin-arm64.zip), so the
...
// (qiaotongagent.key) and the public key (qiaotongagent.pub) into dir. The password comes
...
func genKey(dir string) error {
	...
	keyPath := filepath.Join(dir, "qiaotongagent.key")
	pubPath := filepath.Join(dir, "qiaotongagent.pub")
...
func signFiles(files []string) error {
	...
	for _, f := range files {
		...
		sig := minisign.SignWithComments(priv, data,
			"file:"+filepath.Base(f), "QiaotongAgent desktop release")
		out := f + ".minisig"
