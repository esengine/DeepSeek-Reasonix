...
describe("desktop sidebar new chat button layout", () => {
  it("keeps localized labels on one line in the compact desktop sidebar", () => {
    ...
    expect(cssRule(".side-head .new-btn > span:not(.shortcut)")).toContain("flex: 0 1 auto");
    ...
    expect(css).toContain("@container sidebar (min-width: 280px)");
    expect(css).toContain("@container sidebar (max-width: 279px)");
