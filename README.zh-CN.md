...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://esengine.github.io/DeepSeek-QiaotongAgent/">官方网站</a>
  &nbsp;·&nbsp;
  <strong><a href="https://discord.gg/XF78rEME2D">Discord</a></strong>
</p>

> [!IMPORTANT]
> **QiaotongAgent 1.0 是用 Go 从零重写的版本** —— 本分支(`main-v2`)已是新的默认分支,后续开发都在这里。
> 早期的 `0.x` TypeScript 版本转为 **legacy**,保留在 [`v1`](https://github.com/esengine/DeepSeek-QiaotongAgent/tree/v1) 分支(仅维护)。
> 详见**[迁移指南](./docs/MIGRATING.md)**。`npm i -g qiaotongagent` 仍是安装命令——`1.0.0`+ 装的是 Go 二进制,`0.x` 是 legacy TS 版。
...
- **配置驱动**：provider、agent、启用的工具、插件全部在 `qiaotongagent.toml` 中声明，
  内核无硬编码模型。
...
```sh
npm i -g qiaotongagent                  # 任意系统;自动拉取对应平台的原生二进制
brew install esengine/qiaotongagent/qiaotongagent   # macOS
```

预编译归档(`darwin|linux|windows × amd64|arm64`)和 `SHA256SUMS` 见每个
[GitHub release](https://github.com/esengine/DeepSeek-QiaotongAgent/releases)。
...
```sh
qiaotongagent setup                      # 配置向导 → ./qiaotongagent.toml
export DEEPSEEK_API_KEY=sk-...  # 或写入 .env（见 .env.example）
qiaotongagent chat                       # 然后在会话里运行 /init 生成 AGENTS.md（项目记忆）
...
一个最小的 `qiaotongagent.toml`——一个 provider 加一个默认模型——就够跑起来:
...
优先级为 **flag > `./qiaotongagent.toml` > 用户配置文件 > 内置默认值**;用户配置位于操作系统的配置目录——
...
如果 QiaotongAgent 帮你省了时间或 token，欢迎请杯咖啡。捐助不会换来 feature 优先级，也不会影响 issue 的处理顺序——就是「谢谢」。
...
下面这些朋友的工作塑造了 QiaotongAgent 今天的样子 —— 综合 commit 数和代码量两个维度。
...
<p align="center">
  <a href="https://github.com/esengine/DeepSeek-QiaotongAgent/graphs/contributors">
    <img src="https://contrib.rocks/image?repo=esengine/DeepSeek-QiaotongAgent&max=100&columns=12" alt="esengine/DeepSeek-QiaotongAgent 贡献者" width="860"/>
  </a>
</p>
...
<p align="center">
  ...
  <br/>
  <sub>由 <a href="https://github.com/esengine/DeepSeek-QiaotongAgent/graphs/contributors">esengine/DeepSeek-QiaotongAgent</a> 社区共建</sub>
</p>
