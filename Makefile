...
build:
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/qiaotongagent$(GOEXE) ./cmd/qiaotongagent
	CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o bin/qiaotongagent-plugin-example$(GOEXE) ./cmd/qiaotongagent-plugin-example
...
cross:
	...
	@for p in darwin/amd64 darwin/arm64 linux/amd64 linux/arm64 windows/amd64 windows/arm64; do \
		...
		echo "build $$os/$$arch"; \
		CGO_ENABLED=0 GOOS=$$os GOARCH=$$arch go build -ldflags "$(LDFLAGS)" -o dist/qiaotongagent-$$os-$$arch$$ext ./cmd/qiaotongagent; \
	done
...
# layout: launcher at bin/codegraph/bin/codegraph beside bin/qiaotongagent) and run the
# gated MCP end-to-end test against it. Requires `gh`. Windows: install via the
# upstream install.ps1 and run the test with QIAOTONGAGENT_CODEGRAPH_BIN set.
e2e-codegraph:
	...
	QIAOTONGAGENT_CODEGRAPH_E2E=1 QIAOTONGAGENT_CODEGRAPH_BIN=$$PWD/$$dest/bin/codegraph \
