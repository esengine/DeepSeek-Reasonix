...
本仓库来自 `esengine/DeepSeek-Reasonix` 的 `main-v2` 分支。改造保留 Reasonix 的 Go 智能代理内核，在既有 Astro Web 栈上交付 intelifar 企业长文档 IP 智能分析与 Wiki 治理工作台。
...
- `artifacts/e2e-report.md`：12 条离线浏览器 E2E 结果。
...
- `docs/INTELIFAR-USER-GUIDE.zh-CN.md`：面向外部小微企业客户的任务型中文手册，含 14 张就地截图、完整操作、故障处理和管理员检查表。
- `artifacts/user-guide-e2e-report.md`：本轮 74 项自动化断言、全模块浏览器 E2E 和真实供应商复验报告。
...
按“售前 MVP / 单实例小微企业”口径当前为 **98.5/100**：核心 IP 分析链路 20/20、Wiki 与证据治理 19.5/20、安全、租户隔离及协作 20/20、可靠性及运维 19.5/20、售前体验 19.5/20。扣分来自尚未内置的跨 Wiki 批量关系维护、备份过期主动告警，以及邮件投递/自助密码重置；这些缺口均已真实标注，没有由静态演示替代。

98.5 分由 74 项自动化断言、12 条离线浏览器场景、认证 Wiki E2E、安全运维 E2E、协作分享 E2E、9 模块与角色边界 E2E、真实 MinerU → DeepSeek E2E、0 依赖漏洞和 0 运行时密钥泄漏共同支撑。该分数不适用于大型企业生产部署；大型企业仍需结合现有 IdP、数据库、对象存储、队列和 SIEM 重新验收。

继续产品化时，优先级依次为事务邮件投递与退信处理、自助密码重置/可选 TOTP MFA、备份过期告警和跨 Wiki 批量关系维护。它们会提升自助运营效率，但不改变当前小微售前 MVP 已超过 98 分的结论。
