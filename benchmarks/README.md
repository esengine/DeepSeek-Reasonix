...
`reasonix run --permission-mode=workspace-write --metrics <path> [--model NAME] [--max-steps N] [--profile delivery] [--ablate ARM] <prompt>`
inside a temp copy of the task's `workdir/`; the posture is deliberate so
unattended fixture writes are allowed. It is the default, not the only one —
`-permission` selects another preset for both modes, and the report header then
names a non-default arm.
...
| `-permission` | `workspace-write` | Suite and SWE-bench modes: agent permission preset — `read-only` \| `workspace-write` (unattended default) \| `danger-full-access`. Under `workspace-write` the dynamic-shell gate denies inline interpreter code (`python -c`, `node -e`) because no human can approve it, so a run measures that policy alongside the agent; `danger-full-access` drops the gate for comparisons against harnesses that have none. Never mix postures in one report. |
...
- [`docs/CLI.md`](../docs/CLI.md) — the `reasonix run` flags the e2e harness
  passes through (`--permission-mode`, `--metrics`, `--model`,
