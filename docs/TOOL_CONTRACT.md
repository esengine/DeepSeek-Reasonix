...
| `cron_create` | false | Create a scheduled task. Accepts `cron` — a 5-field cron expression like `*/5 * * * *` (or an interval token like `5m`, `2h`, `1d`, rounded to the nearest clean step) — and `prompt`, the text to run when it fires. Set `one_shot: true` for a single fire that deletes itself (reminders). The task fires between turns while the session is open; list tasks with cron_list, cancel with cron_delete. The session holds at most 50 tasks. |
