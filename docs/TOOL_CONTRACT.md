...
| `regex_replace` | false | Replace text in a file using a Go RE2 regex pattern. Supports capture groups ($1, ${name}) in the replacement and optional flags (i=case-insensitive, m=multiline, s=dotall, U=ungreedy). Returns unified diff on success. Prefer this over edit_file when you need pattern matching instead of exact strings. |
