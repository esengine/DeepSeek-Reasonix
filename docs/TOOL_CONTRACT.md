...
| `pty` | false | Control a persistent interactive pseudo-terminal session (PTY) across turns. Use for REPLs (Python, Node.js), interactive debuggers (gdb, lldb, pdb), maintaining shell state (cd, venv, export), or managing dev servers. Actions: 'start' (spawn session), 'write_line' (execute a shell command line with newline and auto-capture output; governed by Bash permission policies), 'write' (send raw interactive keystrokes/control characters to REPLs/programs), 'read' (read new unread output), 'list' (list active PTY sessions), 'close' (terminate session), 'resize' (change terminal size). |
...
`bash`, `bash_output`, `edit_file`, `kill_shell`, `pty`, `read_file`,
