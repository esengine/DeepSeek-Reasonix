...
| `complete_step` | true | Compatibility only; hidden from default discovery. Record a model declaration for one unambiguous existing todo. Optional evidence is descriptive, not proof; only that item changes, without advancing the next. |
...
| `update_goal` | true | Report the model judgment for the active Goal: continue, complete, or blocked. The optional completion account contains model declarations; real checks remain independent. Normal turn-end and execution identity validation apply; no evaluator or quality gate. |
...
mid-session. The model chooses verification and review from task context. The host enforces action permissions, preapproval Plan write restrictions, leases, overwrite protection, and failed-batch barriers. It does not infer acceptance obligations from paths or require todos before writes. Structured review tools remain available for explicit review requests.
