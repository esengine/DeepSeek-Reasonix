...
最近已检查生产 bundle（KiB）：初始 JS 425.2 / 426.8；外壳 CSS 115.9 / 116.0；
简中60.5 / 60.6；繁中61.4 / 61.5；初始原始资源2330.8 / 2349.4。
...
继续验证 Composer 和实际工作区文件预览节点。此项只完成展示边界，不代表
模型切换与 controller 重建后的 profile 回写所有权已统一。

- App 仍3827行；六个领域 owner、剩余 effect、纯组合层和删除尺寸豁免未完成。
