...
- **接管**（handoff）：serve 主机上的本地窗口可以接管前台会话。此时
  serve 降级为只读镜像，实时转发本地写者的帧；已建立协作连接的写者
  可通过“取回”交还所有权。心跳中断不会强行解除仍存活写者的会话锁。桌面远程标签页此时进入旁观模式并显示收回
  横幅。
...
远端启动的 CLI 会发现工作区的常驻 Serve，并为自己持有的会话注册同步连接。
普通 `--resume`、`--continue` 和交互 `/resume` 都会注册；只有目标会话已由
Serve 持有时才需要 `/takeover`。Serve 稍后启动时会重试发现。桌面端可以打开
同一会话、读取历史并观察实时输出，而不会强制切换其他标签页当前选择的会话。

“取回”会显示已注册 TUI 的主机和 PID，经确认后中断当前工作并交还写锁。
TUI 随后退出该会话；Serve 取得写锁并加载会话后，桌面端才恢复写入。TUI
切换会话时，会交还旧会话的镜像并注册新会话。

旧版 CLI 或未注册进程可能持有会话但没有同步连接。如果租约能够确认占用者
是 Serve 主机上仍在运行的 Reasonix 进程，桌面端会显示主机和 PID，并在确认后
强制停止该进程。Serve 仍会等待操作系统写锁真正释放后再加载会话。无法验证的
占用者保持只读，需要在对应远端终端或窗口中退出。更新磁盘上的程序不会更新
已经运行的 TUI 进程。
...
| 远程标签页进入旁观模式 | 远端终端或窗口持有会话。可取回已注册 TUI 或经验证的本机 Reasonix 占用者；无法验证时需先在对应终端或窗口退出会话，再重新打开 |
