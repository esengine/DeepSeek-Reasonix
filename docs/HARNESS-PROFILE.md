...
## 9. 双模型基准线（2026-07-31 追加）

### 评分标准（依据数学库三重元标准 + Agda ground truth）

| 级别 | 得分 | 判定 |
|------|------|------|
| **L3** | 100 | verify 全过 + answer.txt 与 Agda refl 值一致 |
| **L2** | 70 | 部分断言过，answer 错 |
| **L1** | 40 | 推导方向对但关键语义错（如未 mod 3） |
| **L0** | 20 | 无有效推导 |
...
| 模型 | 通过 | 总得分 | 通过率 |
|------|------|--------|--------|
| **Qwen3.8-Max-Preview** | 29/29 | 2900/2900 | 100% |
| **DeepSeek-V4-Flash** | 28/29 | 2840/2900 | **97.9%** |

### DeepSeek 失败项诊断（delta3-vanish-hard → L1 40 分）

```
模型答案: Δ³((0,0,1)) = (-3, 3, 0)  ← 整数差分
库定理:   Δ³ ≡ 0 在 GF(3) = {0,1,2} 上（mod 3 循环差分）
```

**根因**：DeepSeek 将"离散差分"套用了训练数据中的**连续/整数差分**直觉（−3、3 未归约到 mod 3），而库的 Δ 是 **GF(3) 上的循环差分算子**（系数 1,−3,3,−1 中 −3,3 在模 3 下为 0——"三次截断"定理的核心机制）。Qwen 正确实现了 mod 3 语义。

**意义**：这是测试集区分度的**实证证据**——同一任务，Qwen 100 分、DeepSeek 40 分。训练数据的"标准数学"与机器验证的"离散数学"确实存在可测差异。

### 成本基线（DeepSeek 17 任务，按量计费）

| 任务 | 成本 |
|------|------|
| 最便宜 | perf-hard ¥0.0355 |
| 最贵 | ym-massgap-hard ¥0.2016（21 步，1.1M tokens） |
| 17 任务合计 | **¥0.84** |

### 观察

1. **hard/误区/七大问题档**：DeepSeek 16/17（94%），仅 delta3 因 mod-3 语义失败
2. **ym-massgap-hard 成本最高**（¥0.20）：21 步 + 1.1M prompt tokens，模型反复尝试
3. **weil-rh-hard 缓存最佳**（93%）：长任务中 prefix 稳定性保持
4. **Qwen vs DeepSeek 差异集中在误区档**：Qwen 无失败，DeepSeek 在 Δ³ 上暴露训练数据污染
