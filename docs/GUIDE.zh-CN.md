...
优先级：**flag > 项目 `.reasonix.toml`（或旧版 `./reasonix.toml`）> 用户配置文件 > 内置默认值**。从
**Reasonix v1.8.1** 开始，用户配置位于 macOS/Linux 的
`~/.reasonix/config.toml`，Windows 为 `%AppData%\reasonix\config.toml`；迁移和相关数据路径见
[配置路径](./CONFIG_PATHS.zh-CN.md)。标注为“仅用户/全局”的字段（包括 agent 轮数上限）不会被项目 `.reasonix.toml` 覆盖。
...
自动值中的用户名按以下优先级解析：仓库本地 `logname`（`reasonix.toml`）、用户全局 `logname`
（全局配置）、`$LOGNAME` 环境变量、系统账户。`user` 作为 `logname` 的旧别名被接受；若两者同时设置，
`logname` 生效并记录一条加载警告。
