...
Resolution order: **flag > `./qiaotongagent.toml` > the user config file >
...
# language    = "zh"               # ui language; empty = auto-detect from $LANG / $QIAOTONGAGENT_LANG
...
`qiaotongagent chat` can grant Bash as an exact command or as a conservative command
...
`qiaotongagent run` stays autonomous but still honours `deny`.
...
QiaotongAgent is an MCP client. A `[[plugins]]` entry's `type` selects the transport:
...
exposes. `make build` also produces `bin/qiaotongagent-plugin-example` — a runnable
...
**Already have an `.mcp.json`?** Drop it in the project root and QiaotongAgent
reads it as-is — the `mcpServers` spec (`command`/`args`/`env`, `type`/`url`/
`headers`, `${VAR}` expansion) maps field-for-field onto `[[plugins]]`. Both
sources are merged; on a name collision `qiaotongagent.toml` wins.
...
**Upgrading from `0.x`?** Your old `~/.qiaotongagent/config.json` is still read for its
`mcpServers` (honouring `mcpDisabled`) as a lowest-priority source, so MCP servers
keep working — move them into `qiaotongagent.toml`'s `[[plugins]]` or a `.mcp.json` when
convenient.

## Slash commands

In `qiaotongagent chat`, built-in commands (`/compact`, `/new`, `/clear`, `/rewind`,
...
another branch. **Custom commands** are Markdown files under `.qiaotongagent/commands/`
(project) or `~/.config/qiaotongagent/commands/` (user) — `review.md` becomes
...
`/memory` lists both memory documents (`QIAOTONGAGENT.md` / `AGENTS.md`) and saved
...
Embed `@` references in a message and QiaotongAgent resolves them before sending, as
...
`qiaotongagent setup` keeps first-run minimal: pick provider → keys (every SKU of a
...
The planner sees loaded `QIAOTONGAGENT.md` / `AGENTS.md` memory and a small read-only
...
`./qiaotongagent.toml` only when that repository needs a shared override, such as a
...
automatically: QiaotongAgent first drafts a read-only plan, then waits for approval
...
`/auto-plan off|on` in `qiaotongagent chat` to change the user-level setting, or
`qiaotongagent config auto-plan off|on` from a shell/script. The visible reasoning
language uses the same shape: `/reasoning-language auto|zh|en` in chat, or
`qiaotongagent config reasoning-language auto|zh|en` in a shell/script. Pass
