...
The username in that auto value resolves in this priority order: a repo-local
`logname` (in `reasonix.toml`), then a user-global `logname` (in the global
config), then the `$LOGNAME` environment variable, then the system account.
`user` is accepted as an older alias for `logname`; if both are set, `logname`
wins and a load warning is recorded.
