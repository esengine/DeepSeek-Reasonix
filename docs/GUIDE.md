...
| `/loop [interval] [prompt]` | Runs a prompt repeatedly on a schedule, or manages scheduled loops | `/loop 5m check the deploy` runs the prompt every 5 minutes; `/loop <prompt>` runs a dynamic loop where the agent picks each delay via `schedule_wakeup`; a bare `/loop` uses `loop.md` or a built-in maintenance prompt. Prefix with `--forever` (e.g. `/loop --forever 5m check the deploy`) for an endless loop that never expires (default: tasks expire after 7 days). Fixed loops are cancelled by asking the agent (`cron_delete`); dynamic loops pause on `Esc` and cancel on a second `Esc`. The status bar shows the next scheduled fire as `NEXT JOB <id> <time>`. |
