...
| **完全权限**（`danger-full-access`） | 取消普通文件系统围栏。 | 跳过普通询问规则；显式禁止、受保护应用状态和任务强制限制仍生效。 |
...
普通审批不再提供永久授权。会话授权只保存在内存中，不跨重启或会话分支，可以查询和撤销。切换权限或撤销授权都会推进 permission revision；旧 generation 或旧 revision 创建的回复会被拒绝。
