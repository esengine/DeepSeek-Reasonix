# QiaotongAgent Engineering Spec

> QiaotongAgent is a coding agent: a thin harness driving multiple models, with **all
...
```
qiaotongagent/
├── go.mod / go.sum          # module qiaotongagent; require BurntSushi/toml
├── Makefile                 # build / cross / vet / fmt / test
├── README.md / README.zh-CN.md
├── qiaotongagent.example.toml         # sample config
├── docs/SPEC.md             # this file
├── cmd/qiaotongagent/main.go          # entry; blank-imports built-in providers/tools
├── cmd/qiaotongagent-plugin-example/  # reference MCP stdio plugin (a runnable example)
└── internal/
    ...
    ├── command/             # custom slash commands loaded from .qiaotongagent/commands/*.md
...
- `cmd/qiaotongagent-plugin-example` is a runnable reference stdio server (`echo`,
...
Long tasks eventually fill the model's context window. QiaotongAgent manages this with
...
- The dropped originals are archived under the user config dir
  (`qiaotongagent/archive/<timestamp>.jsonl`; see §5 for its per-OS location), one
...
- **Resolving `Ask`.** The interactive front-end (the chat TUI) prompts the user
  ...
  non-interactive run
  (`qiaotongagent run`, a sub-agent, anything with no TTY / no approver) cannot prompt, so
...
Out of the box (`mode = "ask"`, no rules) `qiaotongagent run` behaves exactly as before
(writers resolve `Ask`→allow with no TTY), while `qiaotongagent chat` now prompts before
...
- **Custom commands** are Markdown files under `.qiaotongagent/commands/` (project) and
  `qiaotongagent/commands/` in your OS config dir (user; see §5); the project dir overrides the user dir on a
...
- Frontmatter is an optional `---`-fenced block of simple `key: value` lines;
  `description` and `argument-hint` are recognised (no YAML dependency — QiaotongAgent
...
Resolution order: **flag > project `./qiaotongagent.toml` > the user config file
> built-in defaults**. The user config lives in your OS config dir — `~/.config/qiaotongagent/`
...
`qiaotongagent.toml` should override them only when the repository needs shared
runtime bounds.

```toml
default_model = "deepseek"   # provider name (→ its default model) or "provider/model"
# language    = "zh"                # ui language tag; empty = auto-detect from $LANG / $QIAOTONGAGENT_LANG

[agent]
system_prompt = "You are QiaotongAgent, a coding agent..."  # or system_prompt_file = "..."
...
command = "qiaotongagent-plugin-example"
args    = []
# env   = { FOO = "bar" }
...
`qiaotongagent setup` writes this default config so the CLI is usable out of the box.
...
`[[plugins]]`; on a name collision `qiaotongagent.toml` wins (it is the more explicit,
QiaotongAgent-specific source). This lets a server already configured for Claude work in
QiaotongAgent unchanged.
...
- Build: `CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=$(VERSION)" -o qiaotongagent ./cmd/qiaotongagent`
...
- Sandbox Phase 1: an OS-level jail for `bash` so commands — not just the
  ...
  command unconfined via the permission gate (in `qiaotongagent run`, the command just
...
- "Always allow" persistence writing learned rules back to project config; a
  per-session permission override flag for `qiaotongagent run`.
