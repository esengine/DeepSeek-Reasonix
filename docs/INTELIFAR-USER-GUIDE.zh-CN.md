...
![资产关系全景：资产、关系和权属信息](../artifacts/ip-asset-graph/01-panorama-overview.png)

图 6：全景网络只展示当前账号有权查看的资产。默认实线表示已经人工确认的关系。
...
“显示待复核”只对内容维护者和管理员开放。开启后，虚线关系是系统从文档中发现、但尚未人工确认的建议，不能直接用于客户承诺、技术变更或权属判断。只读成员只能看到已确认关系，也不会看到自己无权访问的机密资产。

复核关系时：

1. 开启“显示待复核”。
2. 点击一条虚线，查看关系两端、关系类型、置信度、发现方式和证据数量。
3. 回到对应原文核对业务含义。关系真实且方向正确时点击“确认关系”；不成立时点击“拒绝建议”。
4. 已确认关系会参与资产搜索扩展和影响分析；被拒绝的建议会从网络中移除。

![待复核关系：查看依据并确认或拒绝](../artifacts/ip-asset-graph/03-proposed-relations.png)
