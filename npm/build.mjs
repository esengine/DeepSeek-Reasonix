...
for (const t of TARGETS) {
  const name = `@qiaotongagent/cli-${t.node}`;
  const dir = join(STAGE, `cli-${t.node}`);
  const exe = t.goos === "windows" ? "qiaotongagent.exe" : "qiaotongagent";
    ...
    `${JSON.stringify(
      {
        name,
        version,
        description: `qiaotongagent prebuilt binary for ${t.node}.`,
        ...
        repository: {
          type: "git",
          url: "git+https://github.com/esengine/DeepSeek-QiaotongAgent.git",
        },
      },
      null,
      2,
...
const mainDir = join(STAGE, "qiaotongagent");
mkdirSync(mainDir, { recursive: true });
cpSync(join(HERE, "qiaotongagent", "bin"), join(mainDir, "bin"), { recursive: true });
cpSync(join(ROOT, "README.md"), join(mainDir, "README.md"));

const mainPkg = JSON.parse(
  readFileSync(join(HERE, "qiaotongagent", "package.json"), "utf8"),
);
...
// stable to default with a manual `npm dist-tag add qiaotongagent@<ver> latest`.
...
console.log(`publish qiaotongagent@${version} (${distTag})`);
