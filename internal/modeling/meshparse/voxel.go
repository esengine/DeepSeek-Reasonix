...
func Voxelize(m *Mesh, resolution int) (*VoxelModel, error) {
	...
	cells := w * h * d
	// Division compare — cells*len(tris) could overflow (wrap negative) on
	// 32-bit int platforms and defeat the cap.
	if cells > 0 && len(tris) > 0 && len(tris) > 500_000_000/cells {
		return nil, fmt.Errorf("meshparse: voxelize workload %d cells × %d tris exceeds cap (lower resolution or simplify mesh)", cells, len(tris))
	}
	...
	for x := 0; x < w; x++ {
		...
		for y := 0; y < h; y++ {
			...
			for z := 0; z < d; z++ {
				...
				if len(hits) > 0 {
					sort.Float64s(hits)
					// Epsilon scales with t AND the mesh's coordinate magnitude on
					// ALL axes: float64 error in t comes from big-number
					// cancellation (ny*(a.Y-py)/nx etc.), so a mesh translated on
					// Y/Z needs the same widened merge window as X.
...
func rayTriangleNearest(px, py, pz float64, tri [3]Vec3) (tHit float64, ok bool) {
	...
	// flipped). epsUV keeps a ~1e-14 floor at the origin and grows for
	...
	// floor at the origin and a ~45-85× margin over ulp at large coordinates
