// Package memory implements QiaotongAgent's persistent memory. It mirrors Claude
// Code's two-layer model while honoring QiaotongAgent's cache-first architecture:
//
//   - Hierarchical doc memory: QIAOTONGAGENT.md / AGENTS.md files discovered from the
...
const (
	ScopeUser     Scope = "user"     // ~/.config/qiaotongagent/QIAOTONGAGENT.md
	ScopeAncestor Scope = "ancestor" // a QIAOTONGAGENT.md above the project root
	ScopeProject  Scope = "project"  // ./QIAOTONGAGENT.md (committed, shared)
	ScopeLocal    Scope = "local"    // ./QIAOTONGAGENT.local.md (personal, git-ignored)
)

// docNames are the recognized memory filenames at each level, in load order.
// QIAOTONGAGENT.md is ours; AGENTS.md and CLAUDE.md are the cross-tool conventions.
...
var docNames = []string{"QIAOTONGAGENT.md", "AGENTS.md", "CLAUDE.md"}

// localNames are the personal, git-ignored overrides, highest precedence.
var localNames = []string{"QIAOTONGAGENT.local.md", "AGENTS.local.md", "CLAUDE.local.md"}
