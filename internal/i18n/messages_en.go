...
var English = Messages{
	...
	SetKeyHint:      "Set your API key, then run `qiaotongagent chat`.",
	ConfigLabel:     "config",
	ModelsLabel:     "models",
	...
	StepScaffold:    "scaffold qiaotongagent.toml",
	StepSetKey:      "set API key",

	InitHint:       "Project memory (AGENTS.md) is generated in-session: run `qiaotongagent chat`, then `/init` — the model analyzes the codebase and writes it. For configuration, use `qiaotongagent setup`.",
	StepSetKeyHint: "run `qiaotongagent setup`, or export DEEPSEEK_API_KEY=…",
	StepChatDesc:   "interactive session",
	StepRunDesc:    "one-shot task",
	HelpFooter:     "qiaotongagent help · all commands",

	ChatTip:           "Context is kept across turns. Type 'exit' or Ctrl-D to quit.",
	TurnCancelled:     "cancelled — back to prompt",
	NoSessionToResume: "no saved session to resume — start a new one with `qiaotongagent chat`",
	...
	OutputStyleHint:    "set agent.output_style in qiaotongagent.toml to apply one (takes effect next session)",
	ThemeHeader:        "themes:",
	...
	SlashMCPNone:                 "no MCP servers configured — add a [[plugins]] entry in qiaotongagent.toml",
	...
	ArgLanguageAuto: "auto-detect from QIAOTONGAGENT_LANG / locale",
	...
	ListHooksNone:       "hooks: none active — configure in .qiaotongagent/settings.json (project, after trust) or ~/.qiaotongagent/settings.json (global)",
	ListMcpHeader:       "mcp servers",
	ListMcpNone:         "mcp: no servers connected — add one in qiaotongagent.toml ([[plugins]]) or a project .mcp.json",

	MemoryNone:             "memory: none — add with “/remember <note>” or create QIAOTONGAGENT.md in the project root",
	...
	MissingKeyIntro:       "qiaotongagent.toml is ready — just an API key away.",
	WroteFileFmt:          "Wrote %s",
	SetupComplete:         "Setup complete.",
	SetupCancelled:        "setup cancelled.",
	TryHintFmt:            "Try: %s",
	NextHint:              "Next: set your API key (run `qiaotongagent setup` or export DEEPSEEK_API_KEY=...), then run `qiaotongagent run \"your task\"`.",
	...
	SkipStaleCustomEntryFmt:    "skipping stale %q entry from qiaotongagent.toml (pointing at %s) — please remove it from [[providers]]",
	...
	UsageRunHint:              "usage: qiaotongagent run [--model NAME] <task>",
	ErrorPrefix:               "error:",
	...
	ProviderErrAuth:                "Authentication failed (HTTP 401): your API key is missing or unset. Add it to .env or run `qiaotongagent setup`.",
	ProviderErrAuthRejected:        "Authentication failed (HTTP 401): the server rejected your API key. It may be wrong or expired, or the provider hit a transient auth/quota issue — retried with backoff and still failed. Try again shortly, or check the key in .env / run `qiaotongagent setup`.",
	...
	UsageBody: `qiaotongagent — a config- and plugin-driven coding agent (multi-model)

Usage:
  qiaotongagent chat [--model NAME] [-c|--continue] [--resume]   interactive session (multi-turn; -c resumes the latest, --resume picks one)
  qiaotongagent run  [--model NAME] [--max-steps N] [-c|--continue] [--resume PATH] <task>   run one task and exit
  qiaotongagent serve [--model NAME] [--addr HOST:PORT]      serve the session over HTTP+SSE (browser client at /)
  qiaotongagent acp [--model NAME]                           serve Agent Client Protocol over stdio (also: qiaotongagent --acp)
  qiaotongagent setup [path]                                 interactive config wizard; writes qiaotongagent.toml (+ .env)
  qiaotongagent config auto-plan [off|on]                    configure automatic plan mode
  qiaotongagent config reasoning-language [auto|zh|en]        configure visible reasoning language
  qiaotongagent mcp <add|remove|list>                        manage MCP servers in qiaotongagent.toml
  qiaotongagent doctor [--json]                              print redacted local diagnostics
  qiaotongagent bot start|doctor|weixin-login                multi-channel IM bot gateway
  qiaotongagent upgrade [--check] [--force]                   self-update to the latest release
...
Configuration:
  Resolution: flag > ./qiaotongagent.toml > ~/.config/qiaotongagent/config.toml > built-in defaults
  Secrets come from the environment via api_key_env (e.g. DEEPSEEK_API_KEY).
  Run 'qiaotongagent setup' to scaffold a config; see docs/SPEC.md.
`,
}
