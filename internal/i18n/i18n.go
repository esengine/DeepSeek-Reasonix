...
type Messages struct {
	...
	StepScaffold    string // step 1 desc — qiaotongagent setup
	StepSetKey      string // step 2 command label

	// `qiaotongagent init` — points to the in-session /init skill + setup
	InitHint       string
	StepSetKeyHint string // step 2 desc — env var hint
	StepChatDesc   string // qiaotongagent chat step desc
	StepRunDesc    string // qiaotongagent run step desc
	HelpFooter     string // dim footer linking to qiaotongagent help
	...
	WroteFileFmt          string // "Wrote %s" — used for qiaotongagent.toml and .env both
	...
	SkipStaleCustomEntryFmt    string // "skipping stale %q entry from qiaotongagent.toml (pointing at %s) — please remove it"
	...
	UsageRunHint              string // "usage: qiaotongagent run [--model NAME] <task>"
	...
	// `qiaotongagent upgrade` / `qiaotongagent update` — self-update
...
//
// Priority: override > QIAOTONGAGENT_LANG > LC_ALL > LC_MESSAGES > LANG > "en".
...
func envCandidates() []string {
	keys := []string{"QIAOTONGAGENT_LANG", "LC_ALL", "LC_MESSAGES", "LANG"}
