...
var Chinese = Messages{
	...
	SlashHelp:          "命令：/compact · /new · /resume · /rewind · /tree · /branch · /switch · /todo · /verbose · /model（切换模型）· /mcp · /skill · /hooks · /paste-image · /memory · /quit · /help · 以及 skills（/init、/explore …）",
	...
	FetchModelsFailedFmt:    "获取 %s 模型失败: %v (使用预设)",
	SelectModelsLabel:       "选择要启用的 %s 模型",
	FetchModelsUsingPresets: "使用预设模型 (API 不可用)",
	SelectProvidersReveal:   "按 / 显示更多 provider",
	BillingModeLabel:        "选择 %s 计费模式",
	BillingModeTP:           "Token Plan (订阅制)",
	BillingModePPU:          "按量计费 (按 token)",
	MiMoTPDetected:          "检测到 Token Plan API key (tp- 前缀)",
	MiMoPPUDetected:         "检测到 Pay Per Use API key (sk- 前缀)",
