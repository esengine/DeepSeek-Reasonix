...
// a batch, so a turn-wide cache could serve a stale diff. Keyed by (tool name,
// args) so two different tools that happen to share the same args JSON under
// the same ctx never collide; a differing key simply misses and recomputes,
// which is safe.
//
