  ...
  "tasks":{
    ...
    "items":{
      ...
      "properties":{
        ...
        "write_paths":{"type":"array","items":{"type":"string"},"description":"Write targets for this item. When concrete targets are known, prefer individual file paths; declare a whole directory only when the writer may modify the directory broadly. Writers that can run at the same time must declare non-overlapping paths; writers ordered by depends_on may share them. Omitting write_paths claims the whole workspace; two concurrent whole-workspace claims (or any overlap between concurrent writers) fail preflight and start nothing."},
