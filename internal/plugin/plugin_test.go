...
func TestStartAvailableKeepsGoodServers(t *testing.T) {
	...
	bad := Spec{Name: "bad", Command: "qiaotongagent-missing-mcp-binary"}
...
func TestStdioCommandNotFoundSuggestsPATHFix(t *testing.T) {
	...
	host, _ := StartAvailable(ctx, []Spec{{Name: "missing", Command: "qiaotongagent-missing-mcp-binary"}})
	defer host.Close()
	...
	for _, want := range []string{
		`command "qiaotongagent-missing-mcp-binary" not found on PATH`,
