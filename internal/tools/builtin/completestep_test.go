...
func TestCompleteStepVerifiesHostReceipts(t *testing.T) {
	...
	ledger.Record(evidence.Receipt{
		ToolName: "read_file",
		Success:  true,
		Paths:    []string{"internal/tools/builtin/completestep.go"},
		Read:     true,
	})
...
func TestCompleteStepRejectsManualAloneOnceTheTurnHasReceipts(t *testing.T) {
	...
	ledger.Record(evidence.Receipt{
		...
		Mutation: true,
		Write:    true,
		Paths:    []string{"internal/runtime/agent/agent.go"},
	})
	ctx := evidence.WithLedger(context.Background(), ledger)
	...
	_, err = completeStep{}.Execute(ctx, json.RawMessage(`{
		...
		"result":"operator confirmed behavior",
		"evidence":[
			{"kind":"diff","summary":"rewrote the gate","paths":["internal/runtime/agent/agent.go"]},
...
func TestCompleteStepRejectionListsRanCommands(t *testing.T) {
	...
	_, err := completeStep{}.Execute(ctx, json.RawMessage(`{
		"step":"x","result":"y",
		"evidence":[{"kind":"verification","summary":"claimed","command":"go test ./internal/base/nilutil/... ./internal/base/fileutil/..."}]}`))
...
func TestCompleteStepSessionFallbackUsesNormalizedMatching(t *testing.T) {
	msgs := []provider.Message{
		{Role: provider.RoleAssistant, ToolCalls: []provider.ToolCall{{
			ID: "c1", Name: "bash",
			Arguments: `{"command":"go test ./internal/contract/tool/... -count=1 -timeout 60s 2>&1 | tail -10"}`,
		}}},
	...
	if _, err := (completeStep{}).Execute(ctx, json.RawMessage(`{
		"step":"x","result":"y",
		"evidence":[{"kind":"verification","summary":"tool tests pass","command":"go test ./internal/contract/tool/... -count=1 -timeout 60s"}]}`)); err != nil {
