...
// separate from the `qiaotongagent setup` wizard (cli) so a GUI can apply one setting at a
...
// SetLanguage pins the CLI UI/model language; empty/auto clears the override so runtime detection falls back to QIAOTONGAGENT_LANG / locale.
...
// .mcp.json-only server would otherwise be serialized into qiaotongagent.toml or the
...
func pluginTOMLSourcePath(name string) string {
	for _, path := range []string{"qiaotongagent.toml", userConfigPath()} {
...
// half-written qiaotongagent.toml that fails to parse on next load. Parent directories
...
func SaveMinimalProjectAutoPlan(path, mode string) (string, error) {
	...
	body := fmt.Sprintf(`# QiaotongAgent project configuration.
...
func writeConfigFile(path, body string) error {
	...
	tmp, err := os.CreateTemp(dir, ".qiaotongagent.*.toml.tmp")
	if err != nil {
...
// (SourcePath), or to ./qiaotongagent.toml when none exists yet — the conventional
...
// SaveForRoot saves the config to root's qiaotongagent.toml, falling back to the
// user's global config when root has no existing qiaotongagent.toml.
