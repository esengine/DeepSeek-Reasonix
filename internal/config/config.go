...
func Default() *Config {
	return &Config{
		...
		Providers: []ProviderEntry{
			{Name: "deepseek", Kind: "openai", BaseURL: "https://api.deepseek.com", Models: []string{"deepseek-v4-flash", "deepseek-v4-pro"}, Default: "deepseek-v4-flash", ModelsURL: "https://api.deepseek.com/models", APIKeyEnv: "DEEPSEEK_API_KEY", BalanceURL: "https://api.deepseek.com/user/balance", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.02, Input: 1, Output: 2, Currency: "¥"}, Effort: "high"},
			{Name: "mimo-tp", Kind: "openai", BaseURL: "https://token-plan-cn.xiaomimimo.com/v1", Models: []string{"mimo-v2.5", "mimo-v2.5-pro"}, Default: "mimo-v2.5", ModelsURL: "https://token-plan-cn.xiaomimimo.com/v1/models", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.025, Input: 3, Output: 6, Currency: "¥"}},
			{Name: "mimo-ppu", Kind: "openai", BaseURL: "https://api.xiaomimimo.com/v1", Models: []string{"mimo-v2.5", "mimo-v2.5-pro"}, Default: "mimo-v2.5", ModelsURL: "https://api.xiaomimimo.com/v1/models", APIKeyEnv: "MIMO_API_KEY", ContextWindow: 1_000_000, Price: &provider.Pricing{CacheHit: 0.02, Input: 1, Output: 2, Currency: "¥"}},
		},
	}
}
...
func Load() (*Config, error) {
	...
	// Auto-fetch models from models_url for providers that have it set.
	for i := range cfg.Providers {
		p := &cfg.Providers[i]
		if p.ModelsURL != "" && len(p.Models) == 0 && p.Model == "" {
			if models, err := fetchModelsFromURL(p.ModelsURL, p.APIKey()); err == nil && len(models) > 0 {
				p.Models = models
			}
		}
	}
