...
func (t *installSkillTool) Description() string {
	scope := "'global' (only option — no project workspace) writes to ~/.qiaotongagent/skills/."
	if t.store.HasProjectScope() {
		scope = "'project' (default) writes to <repo>/.qiaotongagent/skills/ (this workspace only); 'global' writes to ~/.qiaotongagent/skills/ (every project)."
	}
...
"type":"object",
"properties":{
  "name":{"type":"string","description":"Identifier — letters/digits/_/-/., 1-64 chars, starts alphanumeric. Becomes the skill folder name under ~/.qiaotongagent/skills/<name>/SKILL.md."},
