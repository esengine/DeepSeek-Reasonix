...
func TestApprovalRequestCarriesRenameFileDiff(t *testing.T) {
	...
	if captured.ChangeKind != string(diff.Rename) {
		t.Errorf("Approval.ChangeKind = %q, want %q", captured.ChangeKind, diff.Rename)
	}
...
func TestApprovalRequestNoFileDiffForNonPreviewer(t *testing.T) {
	...
	if captured.ChangeKind != "" {
		t.Errorf("Approval.ChangeKind = %q, want empty for non-Previewer", captured.ChangeKind)
	}
	if captured.Diff != "" || captured.SrcPath != "" || captured.DstPath != "" {
		t.Errorf("non-Previewer approval leaked preview fields: %+v", captured)
	}
}
