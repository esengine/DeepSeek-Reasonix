...
func TestSlashArgItems(t *testing.T) {
	...
	if !has(items, "auto") || !has(items, "high") || !has(items, "max") || !has(items, "disabled") || has(items, "off") {
		t.Errorf("/effort should offer auto/high/max/disabled; got %v", labelsOf(items))
	}
	// /auto-plan
