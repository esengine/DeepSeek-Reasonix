...
func TestClassifyRef(t *testing.T) {
	...
	files := map[string]bool{
		"src/main.go": true,
		"README.md":   true,
		".qiaotongagent/attachments/clipboard-20260601-010203.000000.png": true,
		".qiaotongagent/attachments/clipboard-20260601-010203.000000.yml": true,
		".qiaotongagent/attachments/clipboard-20260601-010203.000000.zip": true,
	}
	...
	cases := []struct {
		...
		{".qiaotongagent/attachments/clipboard-20260601-010203.000000.png", true, refImage},
		{".qiaotongagent/attachments/clipboard-20260601-010203.000000.yml", true, refFile},
		{".qiaotongagent/attachments/clipboard-20260601-010203.000000.zip", true, refFile},
...
func TestResolveRefsAttachmentKinds(t *testing.T) {
	temp := t.TempDir()
	attachmentsDir := filepath.Join(temp, ".qiaotongagent", "attachments")
	if err := os.MkdirAll(attachmentsDir, 0o755); err != nil {
		t.Fatal(err)
	}
	ymlRef := filepath.ToSlash(".qiaotongagent/attachments/config.yml")
	zipRef := filepath.ToSlash(".qiaotongagent/attachments/archive.zip")
	pngRef := filepath.ToSlash(".qiaotongagent/attachments/shot.png")
	if err := os.WriteFile(filepath.Join(temp, filepath.FromSlash(ymlRef)), []byte("name: qiaotongagent\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(block, `<file path="`+ymlRef+`">`) || !strings.Contains(block, "name: qiaotongagent") {
...
func TestWorkspaceImageRefsOnlyTreatAttachmentsAsImages(t *testing.T) {
	...
	attachment := filepath.Join(workspace, ".qiaotongagent", "attachments", "shot.png")
	...
	if refs[1].kind != refImage || refs[1].path != ".qiaotongagent/attachments/shot.png" {
