...
// Trust gates project hooks. A project's .qiaotongagent/settings.json can run
...
// root. The trust flag lives in user-global state (~/.qiaotongagent/trust.json),
// NOT in the project file itself — an attacker controls the latter. Global
// hooks (~/.qiaotongagent/settings.json) are the user's own and always run.

// TrustFilename is the user-global trust store under ~/.qiaotongagent.
...
// TrustPath is ~/.qiaotongagent/trust.json (homeDir overrides ~).
