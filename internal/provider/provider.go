...
type Message struct {
	...
	// DeferredTodoCompletions is durable host-only state attached to a successful
...
// before the serial position reached it. Level is retained so a stale
// completion cannot be applied after an identity-preserving but structurally
...
// DeferredTodoCompletionState is a snapshot of all deferred completions after
// one successful Todo transition. An empty Items slice is meaningful: it
// clears an older deferred snapshot during transcript replay.
type DeferredTodoCompletionState struct {
	Items []DeferredTodoCompletion `json:"items"`
}
