...
func TestResolveCanonicalSessionQuery(t *testing.T) {
	dir := t.TempDir()
	sessionDir := filepath.Join(dir, "sessions")
	v4root := filepath.Join(dir, "sessions-v4")
	if err := os.MkdirAll(sessionDir, 0o700); err != nil {
	...
	createCanonicalTestSession(t, v4root, "aaaa1111", "canonical hello")
	waitForCatalogMetadata(t, sessionDir, "aaaa1111")

	if target, ok := resolveCanonicalSessionQuery(sessionDir, "aaaa1111"); !ok || target.ref.SessionID != "aaaa1111" {
		t.Fatalf("exact id match = (%+v, %v)", target, ok)
	}
	if target, ok := resolveCanonicalSessionQuery(sessionDir, "aaaa"); !ok || target.ref.SessionID != "aaaa1111" {
		t.Fatalf("unique prefix match = (%+v, %v)", target, ok)
	}
	if target, ok := resolveCanonicalSessionQuery(sessionDir, "canonical hello"); !ok || target.ref.SessionID != "aaaa1111" {
		t.Fatalf("preview match = (%+v, %v)", target, ok)
	}
	if _, ok := resolveCanonicalSessionQuery(sessionDir, "zzzz"); ok {
		t.Fatal("unknown query matched a canonical session")
	}
}
