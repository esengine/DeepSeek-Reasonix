...
func TestMainManagerFollowsTranscriptWithoutTopPadding(t *testing.T) {
	...
	m = m0.(chatTUI)
	m.wrappedLines = []string{"qiaotongagent chat", "› /mcp"}
	...
	if !strings.Contains(lines[0], "qiaotongagent chat") || !strings.Contains(lines[1], "/mcp") {
...
func TestAutoPlanCommandWritesUserConfigNotProjectConfig(t *testing.T) {
	isolateUserConfig(t)
	projectPath := filepath.Join(mustGetwd(t), "qiaotongagent.toml")
...
func TestLanguageCommandAutoClearsLowerPriorityUserOverride(t *testing.T) {
	...
	if err := projectCfg.SaveTo("qiaotongagent.toml"); err != nil {
