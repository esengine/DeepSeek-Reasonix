...
func runAgent(args []string, version string) int {
	...
	cont, resume, copySession := registerResumeFlags(fs, "resume a specific session file (non-interactive; takes precedence over --continue)", "")
...
func chatREPL(args []string, version string) int {
	...
	profileFlag := fs.String("profile", "balanced", "runtime profile: economy | balanced | delivery")
	maxSteps := fs.Int("max-steps", 0, "one-off max tool-call rounds (0 = automatic)")
	cont, resume, copySession := registerResumeFlags(fs, "resume by session ID/query, or open the picker when no value is given", "r")
