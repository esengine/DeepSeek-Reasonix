...
func resolveSessionQuery(dir, query string) (cliResumeTarget, error) {
	...
	switch len(matches) {
	case 0:
		// No legacy transcript matched; the query may name a final-format
		// session identity from the same workspace's catalog.
		if target, ok := resolveCanonicalSessionQuery(dir, query); ok {
			return target, nil
		}
		return cliResumeTarget{}, fmt.Errorf("no session matches %q", query)
	case 1:
