...
// mcp.go holds the MCP server-management surface shared by the `qiaotongagent mcp`
...
// mcpCommand implements `qiaotongagent mcp <add|remove|list>`. It edits config only
...
func mcpList() int {
	...
	if bin, ok := codegraph.Resolve(cfg.Codegraph.Path); ok {
		...
		fmt.Printf("%-16s (built-in, not installed)%s  run `qiaotongagent codegraph install`", "codegraph", codegraphMeta)
...
func mcpRemoveCLI(args []string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: qiaotongagent mcp remove <name>")
		return 2
	}
	name := args[0]
	cfg, err := config.Load()
...
func mcpUsage() {
	fmt.Println(`Manage MCP servers (persisted to qiaotongagent.toml).

Usage:
  qiaotongagent mcp list
  qiaotongagent mcp add <name> <command> [args...]        stdio server
  qiaotongagent mcp add <name> --http <url> [--header K=V] remote (Streamable HTTP)
  qiaotongagent mcp add <name> --sse  <url>               remote (legacy SSE)
  qiaotongagent mcp import                                import Codex-enabled servers from cc-switch
  qiaotongagent mcp remove <name>
...
Examples:
  qiaotongagent mcp add fs npx -y @modelcontextprotocol/server-filesystem .
  qiaotongagent mcp add stripe --http https://mcp.stripe.com --header "Authorization=Bearer $STRIPE_KEY"
