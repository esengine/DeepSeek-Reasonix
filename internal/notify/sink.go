...
func message(cfg config.NotificationsConfig, e event.Event) (Message, bool) {
	...
	case event.TurnDone:
		if cfg.TurnDone {
			if e.Err != nil {
				return Message{Title: "QiaotongAgent", Body: "Turn failed"}, true
			}
			return Message{Title: "QiaotongAgent", Body: "Turn finished"}, true
		}
	case event.ApprovalRequest:
		if cfg.ApprovalRequest {
			return Message{Title: "QiaotongAgent", Body: "Approval needed"}, true
		}
	case event.AskRequest:
		if cfg.AskRequest {
			return Message{Title: "QiaotongAgent", Body: "Question needs your answer"}, true
		}
	}
