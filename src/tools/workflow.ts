    ...
    parameters: {
      ...
      properties: {
        script: {
          type: "string",
          description:
            'Required raw JavaScript workflow script, with no Markdown fences. First statement must be `export const meta = { name, description, phases? }`. Available globals: agent(), parallel(), pipeline(), verifyFindings(), adversarialReview(), synthesize(), phase(), log(), args, cwd, budget. For large audits/reviews, write scripts as scope/inventory phase -> parallel agents -> verifier/adversarial review -> synthesis. Use `agent("prompt", { label, type })` or `agent({ instruction, label/name, type })`. agent() and synthesize() return string|null directly; assign their return value as the output text and do not read result.output or .output. Use `parallel([() => agent(...), () => agent(...)])` for independent fan-out; writing `await agent(...); await agent(...)` is serial. Control concurrency with the workflow tool concurrency argument or `/workflows run --concurrency`; do not pass concurrency options to parallel(). Use pipeline() for dependency-heavy stages. Use verifyFindings([...]) for independent verifier subagents, adversarialReview(result) to challenge a draft, and synthesize(inputs) for final synthesis. For cost-sensitive runs, verify only high-risk or sampled findings. The workflow must call agent() at least once.',
        },
        args: {
