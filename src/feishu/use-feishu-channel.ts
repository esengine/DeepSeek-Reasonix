...
export function useFeishuChannel({
  ...
  const connect = useCallback(
    async (args: readonly string[]): Promise<string> => {
      ...
      const ownerOpenId = args[2]?.trim() || existing.ownerOpenId || "";

      if (!appId || !appSecret || !ownerOpenId) {
        return beginConnectSetup({
          appId: appId || undefined,
          appSecret: appSecret || undefined,
          ownerOpenId: ownerOpenId || undefined,
  ...
  const parseSubmit = useCallback(
    (raw: string) => {
      ...
      if (!fromFeishu && pendingConnectSetupRef.current) {
        ...
        if (text.startsWith("/")) {
          log.pushInfo(formatFeishuSetupPrompt(pending.step));
          return { handled: true, fromFeishu, text };
        }
        ...
        if (pending.step === "appSecret") {
          pending.appSecret = text;
          pending.step = "ownerOpenId";
          log.pushInfo(formatFeishuSetupPrompt("ownerOpenId"));
          return { handled: true, fromFeishu, text };
        }

        pending.ownerOpenId = text;
        pendingConnectSetupRef.current = null;
  ...
  useEffect(() => {
    if (feishuSubmitRef) {
      feishuSubmitRef.current = (text: string) => setQueuedSubmit(text);
    }
    if (feishuErrorRef) {
      feishuErrorRef.current = (msg: string) => log.pushWarning("Feishu", msg);
    }
    return () => {
      if (feishuSubmitRef) feishuSubmitRef.current = null;
      if (feishuErrorRef) feishuErrorRef.current = null;
      pendingConnectSetupRef.current = null;
    };
