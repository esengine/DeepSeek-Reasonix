...
export const EN: TranslationSchema = {
  ...
  statusBar: {
    ...
    workspace: "\u25b8 workspace",
    relaunchDir: "  (relaunch with --dir <path> to switch)",
    feedback: "/feedback",
    recordingGlyph: "\u25CFREC",
    mb: " MB",
    evt: " evt",
  ...
  composer: {
    ...
    hint: "\u23ce send  \u00b7  shift/alt+\u23ce newline  \u00b7  \u2191\u2193 scroll  \u00b7  ^P/^N history  \u00b7  esc abort  \u00b7  ctrl-c quit",
  ...
  editMode: {
    ...
    runningJobs: "  \u00b7  \u23f5 {count} job{count === 1 ? \"\" : \"s\"}",
    ...
    toResume: " to resume  ",
    paused: "paused",
    summaryFindings: "summarising findings\u2026",
    exploringTask: "exploring task\u2026",
    thinkingDots: "thinking\u2026",
    workingThrough: "working through tools\u2026",
  ...
  shellConfirm: {
    ...
    longRunning: "long-running process \u2014 keeps running after approval, /kill to stop",
    ...
    denyContextLabel: "Deny \u2014 provide context",
    denyContextPlaceholder: "optional",
    denyContextFooter: "type context  \u00b7  \u23ce submit with reason  \u00b7  esc skip (deny without reason)",
    awaiting: "awaiting",
    footer: "\u2191\u2193 pick  \u00b7  \u23ce confirm  \u00b7  Tab add context  \u00b7  esc cancel",
  },
  editConfirm: {
    footer: "[y/Enter] apply  \u00b7  [n] reject with reason  \u00b7  [a] apply rest  \u00b7  [A] flip AUTO  \u00b7  [\u2191\u2193/Space] scroll  \u00b7  [Esc] abort",
    newTag: "NEW",
    editTag: "EDIT",
    minusPlusLines: "-{removed} +{added} lines",
    viewingRange: "viewing {start}-{end}/{total}",
    denyContextLabel: "Deny \u2014 provide context",
    denyContextPlaceholder: "optional",
    denyContextFooter: "\u23ce submit  \u00b7  esc skip (deny without reason)",
    oldHeader: "  - old",
    newHeader: "  + new",
    sideBySideDesc: "   side-by-side \u00b7 removed lines on the left, added on the right \u00b7 paired by offset",
    linesAbove: "  \u2191 {count} line{count === 1 ? \"\" : \"s\"} above  (\u2191/k or PgUp)",
    linesBelow: "  \u2193 {count} line{count === 1 ? \"\" : \"s\"} below  (\u2193/j or Space/PgDn)",
  },
  choiceConfirm: {
    typeOwn: "Let me type my own answer",
    typeOwnDesc: "None of the above fits \u2014 type a free-form reply. The model reads it verbatim.",
    cancel: "Cancel \u2014 drop the question",
  ...
  cardLabels: {
    ...
    topTools: "top tools",
    logMsgs: "log msgs",
    hits: "{count} hit{count === 1 ? \"\" : \"s\"} \u00b7 {files} file{files === 1 ? \"\" : \"s\"}",
    files: "files",
    moreHits: "\u22ee +{count} more hit{count === 1 ? \"\" : \"s\"}",
    earlierLines: "\u22ee {count} earlier line{count === 1 ? \"\" : \"s\"} (use /tool to read full)",
    stackTrace: "stack trace",
    earlierStackLines: "\u22ee {count} earlier stack line{count === 1 ? \"\" : \"s\"} hidden",
    retryCount: "\u21bb {attempt}/{max}",
    retries: "retries",
    rejected: "rejected",
    exit: "exit {code}",
    agent: "agent \u00b7 {name}",
    ...
    aboveHint: "\u22ef {parts} above \u00b7 /reasoning last",
    ...
    upcoming: "upcoming",
    checks: "{count} check{count === 1 ? \"\" : \"s\"} \u00b7 {ok} passed{warn > 0 ? \" \u00b7 \" + warn + \" warn\" : \"\"}{fail > 0 ? \" \u00b7 \" + fail + \" fail\" : \"\"}",
    passed: "passed",
    warnPill: "warn",
    failPill: "FAIL",
    more: "\u22ee +{count} more",
    ...
    pilcrow: "\u00b6",
    elapsedSec: "{secs}s",
    addedLines: "+{count} line{count === 1 ? \"\" : \"s\"}",
    removedLines: "-{count} line{count === 1 ? \"\" : \"s\"}",
  },
  mcp: {
    lifecycleHandshake: "handshake\u2026",
    lifecycleConnected: "connected",
    lifecycleFailed: "failed",
    lifecycleDisabled: "disabled",
    lifecycleReconnect: "reconnect\u2026",
    lifecycleInitDetail: "initialise \u2192 tools/list \u2192 resources/list",
    lifecycleDisableDetail: "via /mcp disable {name}",
    lifecycleReconnectDetail: "tearing down \u00b7 re-handshake \u00b7 listing tools",
    ...
    readResource: "Read one: `/resource <uri>` \u2014 or use Tab in the picker.",
    noPrompts: "No prompts on any connected MCP server (or no servers connected). `/mcp` shows the current set.",
    fetchPrompt: "Fetch one: `/prompt <name>` \u2014 args are not supported yet; prompts with required args will surface an error from the server.",
    resourceContent: "Resource {uri} ({count} content block{count === 1 ? \"\" : \"s\"}):",
    binaryContent: "[binary \u00b7 ~{bytes} bytes \u00b7 base64]",
    promptHeader: "Prompt {name}{desc}",
    messages: "({count} message{count === 1 ? \"\" : \"s\"})",
    truncated: "[\u2026truncated {count} chars; full contents available via McpClient.readResource in library mode.]",
    readResourceFailed: "readResource failed",
    getPromptFailed: "getPrompt failed",
    browserTitle: "\u25c8 MCP browser",
    noServers: "No MCP servers attached. Run `reasonix setup` to pick some, or launch with --mcp.",
    browserFooter: "\u2191\u2193 pick \u00b7 [r] reconnect \u00b7 [d] disable \u00b7 esc quit",
    hubTitle: "\u25c8 MCP",
    summary: "   tab to switch",
  },
  loop: {
    usage: "usage: /loop <interval> <prompt>   (interval = 5s..6h, e.g. 30s, 5m, 1h)\n       /loop stop                  (cancel an active loop)\n       /loop                       (show active-loop status)",
    errorMissingPrompt: "usage: /loop {cmd} <prompt>   \u2014 interval is fine but the prompt is missing.",
    statusLine: "loop: `{preview}` \u00b7 {when} \u00b7 iter {iter}",
  ...
  sessionPicker: {
    ...
    noSessions: "  no saved sessions in this workspace yet \u2014 press ",
    footer: "\u2191\u2193 pick  \u00b7  \u23ce open  \u00b7  [n] new  \u00b7  [d] delete  \u00b7  [r] rename  \u00b7  esc quit",
    renameFooter: "  \u23ce confirm rename  \u00b7  esc cancel",
    newFooter: "  \u23ce new session  \u00b7  esc quit",
    turns: "{count} turn{count === 1 ? \"\" : \"s\"}",
    messages: "{count} message{count === 1 ? \"\" : \"s\"}",
    open: "open",
    new: "new",
    delete: "delete",
  ...
  slashSuggestions: {
    ...
    advancedHidden: "  + {count} advanced  \u00b7  type a letter to search",
  ...
  atMentions: {
    ...
    entries: "{count} {count === 1 ? \"entry\" : \"entries\"}",
    footerBrowse: "\u2191\u2193 navigate \u00b7 Tab drill into folder \u00b7 \u23ce insert \u00b7 esc cancel",
    footerInsert: "\u2191\u2193 navigate \u00b7 Tab / \u23ce insert as @path \u00b7 esc cancel",
    scanned: "searching\u2026 {scanned} scanned \u00b7 {count} match{count === 1 ? \"\" : \"s\"}",
  },
  appStrings: {
    loopStatus: "\u25b8 {status} \u00b7 /loop stop or type to cancel",
    editRejected: "User rejected this edit to {path}{context}. Don't retry the same SEARCH/REPLACE \u2014 either try a different approach or ask the user what they want instead.",
    walkCodeOnly: "/walk is only available inside `reasonix code`.",
    walkNothing: "nothing pending \u2014 nothing to walk through.",
    walkStarting: "\u25b8 walking {count} edit block{count === 1 ? \"\" : \"s\"} \u2014 y apply \u00b7 n reject \u00b7 a apply rest \u00b7 A flip to AUTO \u00b7 Esc cancels (keeps remaining queued).",
    loopBusy: "loop is busy with a turn",
    tuiNotReady: "TUI not ready",
    semanticRepointed: "\u25b8 semantic_search re-pointed at {path}",
    semanticDisabled: "\u25b8 semantic_search disabled (no compatible index in {path})",
    semanticRebootstrapFailed: "\u25b8 semantic_search re-bootstrap failed: {message}",
    workspaceSwitched: "\u25b8 workspace switched to {path}",
    replayStatus: "Replay #{index}/{total} \u00b7 {time}{title}",
    restoredPrefix: "\u25b8 restored \"{name}\" ({id}), from {when}",
    restoredWrote: "  \u00b7 wrote {count} file{count === 1 ? \"\" : \"s\"}",
    restoredRemoved: "  \u00b7 removed {count} file{count === 1 ? \"\" : \"s\"} (didn't exist at checkpoint)",
    switchSession: "\u25b8 to switch to \"{name}\", quit and run: reasonix chat --session {name}",
    freshSession: "\u25b8 to start a fresh session, quit and run: reasonix chat (no --session flag)",
    modelSet: "\u25b8 model: {id}",
    presetSet: "\u25b8 preset: {name} \u00b7 {model}",
  },
};
