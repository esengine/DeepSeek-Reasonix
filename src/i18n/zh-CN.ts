...
export const zhCN: TranslationSchema = {
  ...
  composer: {
    ...
    hint: "⏎ 发送  ·  ⇧⏎ 换行  ·  ↑↓ 滚动  ·  ^P/^N 历史  ·  Esc 中止  ·  Ctrl+C 退出",
  ...
  editMode: {
    ...
    toResume: " 继续  ",
    paused: "已暂停",
    summaryFindings: "正在总结发现…",
    exploringTask: "正在探索任务…",
    thinkingDots: "思考中…",
    workingThrough: "正在处理工具…",
    workingLabel: "工作中",
    subAgentPrefix: "子代理",
    anonymous: "匿名",
    queuedDots: "排队中…",
    runningLabel: "运行中",
    summarisingLabel: "总结中",
    subagents: "子代理",
    moreRunning: "  +{count} 个更多运行中…",
  },
  shellConfirm: {
    title: "Shell 命令",
    bgTitle: "后台进程",
    subtitle: "模型想要执行一条 Shell 命令",
    longRunning: "长时间运行 — 批准后持续运行，/kill 可停止",
    allowOnce: "允许一次",
    allowOnceDesc: "执行此命令，下次再问",
    allowAlways: "始终允许",
    allowAlwaysDesc: "记住 `{prefix}`，本项目内不再询问",
    deny: "拒绝",
    denyDesc: "按 Tab 添加上下文，告诉模型原因",
    denyContextLabel: "拒绝 — 提供原因",
    denyContextPlaceholder: "可选",
    denyContextFooter: "输入原因 · ⏎ 提交 · Esc 跳过（直接拒绝）",
    awaiting: "等待中",
    footer: "↑↓ 选择 · ⏎ 确认 · Tab 添加说明 · Esc 取消",
  },
  editConfirm: {
    footer: "[y/Enter] 应用 · [n] 拒绝并说明 · [a] 应用剩余 · [A] 切换 AUTO · [↑↓/Space] 滚动 · [Esc] 中止",
    newTag: "新增",
    editTag: "编辑",
    minusPlusLines: "-{removed} +{added} 行",
    viewingRange: "正在查看 {start}-{end}/{total}",
    denyContextLabel: "拒绝 — 提供原因",
    denyContextPlaceholder: "可选",
    denyContextFooter: "⏎ 提交 · Esc 跳过（直接拒绝）",
    oldHeader: "  旧内容",
    newHeader: "  新内容",
    sideBySideDesc: "   左右对比 · 左侧删除，右侧新增 · 按偏移配对",
    linesAbove: "  ↑ 上方 {count} 行（↑/k 或 PgUp）",
    linesBelow: "  ↓ 下方 {count} 行（↓/j 或 Space/PgDn）",
  },
  choiceConfirm: {
    typeOwn: "自定义回答",
    typeOwnDesc: "以上选项都不合适 — 输入自由格式回复，模型会原样读取。",
    cancel: "取消 — 放弃问题",
  ...
  cardLabels: {
    ...
    topTools: "Top 工具",
    logMsgs: "日志消息",
    hits: "{count} 条结果 · {files} 个文件",
    files: "个文件",
    moreHits: "⋮ +{count} 条更多结果",
    ...
    aboveHint: "⋯ {parts} 在上方 · /reasoning last",
    ...
    inProgress: "← 进行中",
    upcoming: "待处理",
    checks: "{count} 项检查 · {ok} 通过{warn > 0 ? \" · \" + warn + \" 警告\" : \"\"}{fail > 0 ? \" · \" + fail + \" 失败\" : \"\"}",
    passed: "通过",
    warnPill: "警告",
    failPill: "失败",
    more: "⋮ +{count} 更多",
  ...
  mcp: {
    lifecycleHandshake: "握手…",
    lifecycleConnected: "已连接",
    lifecycleFailed: "失败",
    lifecycleDisabled: "已禁用",
    lifecycleReconnect: "重连…",
    lifecycleInitDetail: "初始化 → 工具列表 → 资源列表",
    lifecycleDisableDetail: "通过 /mcp disable {name}",
    lifecycleReconnectDetail: "断开旧连接 · 重新握手 · 获取工具列表",
    slowToast: "⚠ MCP `{name}` 响应慢 · 最近 {sampleSize} 次调用 p95 {seconds}s",
    noResources: "没有已连接的 MCP 服务器暴露资源（或没有已连接的服务器）。`/mcp` 显示当前状态。",
    readResource: "读取：`/resource <uri>` — 或在选择器中使用 Tab。",
    noPrompts: "没有已连接的 MCP 服务器暴露提示（或没有已连接的服务器）。`/mcp` 显示当前状态。",
    fetchPrompt: "获取：`/prompt <name>` — 暂不支持参数；带必填参数的提示会返回服务端错误。",
    resourceContent: "资源 {uri}（{count} 个内容块）：",
    binaryContent: "[二进制 · ~{bytes} 字节 · base64]",
    promptHeader: "提示 {name}{desc}",
    messages: "（{count} 条消息）",
    truncated: "[…已截断 {count} 字符；完整内容可在库模式下通过 McpClient.readResource 获取。]",
    readResourceFailed: "读取资源失败",
    getPromptFailed: "获取提示失败",
    browserTitle: "◈ MCP 浏览器",
    noServers: "没有已连接的 MCP 服务器。运行 `reasonix setup` 选择一些，或使用 --mcp 启动。",
    browserFooter: "↑↓ 选择 · [r] 重连 · [d] 禁用 · Esc 退出",
    hubTitle: "◈ MCP",
    summary: "   Tab 切换",
  },
  loop: {
    usage: "用法：/loop <间隔> <提示词>   （间隔 = 5s..6h，例如 30s、5m、1h）\n       /loop stop                  （取消活动循环）\n       /loop                       （显示循环状态）",
    errorMissingPrompt: "用法：/loop {cmd} <提示词>   — 间隔没问题，但缺少提示词。",
    statusLine: "循环：`{preview}` · {when} · 第 {iter} 轮",
    firingNow: "立即执行",
    nextIn: "下次执行：",
  },
  ctxBreakdown: {
  ...
  sessionPicker: {
    title: "选择会话",
    noSessions: "  此工作区暂无已保存的会话 — 按 ",
    footer: "↑↓ 选择 · ⏎ 打开 · [n] 新建 · [d] 删除 · [r] 重命名 · Esc 退出",
    renameFooter: "  ⏎ 确认重命名 · Esc 取消",
    newFooter: "  ⏎ 新建会话 · Esc 退出",
    turns: "{count} 轮",
    messages: "{count} 条消息",
    open: "打开",
    new: "新建",
  ...
  slashSuggestions: {
    ...
    advancedHidden: "  + {count} 个高级命令 ·  输入字母搜索",
  ...
  atMentions: {
    ...
    scanned: "搜索中… 已扫描 {scanned} 个 · {count} 个匹配",
  },
  appStrings: {
    loopStatus: "▸ {status} · /loop stop 或输入内容取消",
    editRejected: "用户拒绝了对 {path}{context} 的编辑。不要重试相同的 SEARCH/REPLACE — 换一种方式或询问用户想要什么。",
    walkCodeOnly: "/walk 仅在 `reasonix code` 中可用。",
    walkNothing: "没有待处理的编辑 — 没有需要逐块审阅的内容。",
    walkStarting: "▸ 正在逐块审阅 {count} 个编辑块 — y 应用 · n 拒绝 · a 应用剩余 · A 切换 AUTO · Esc 取消（保留剩余排队）",
    loopBusy: "循环正在执行中",
    tuiNotReady: "TUI 未就绪",
    semanticRepointed: "▸ semantic_search 已重新指向 {path}",
    semanticDisabled: "▸ semantic_search 已禁用（{path} 中没有兼容的索引）",
    semanticRebootstrapFailed: "▸ semantic_search 重新初始化失败：{message}",
    workspaceSwitched: "▸ 工作区已切换到 {path}",
    replayStatus: "回放 #{index}/{total} · {time}{title}",
    restoredPrefix: "▸ 已恢复 \"{name}\"（{id}），来自 {when}",
    restoredWrote: "  · 写回了 {count} 个文件",
    restoredRemoved: "  · 移除了 {count} 个文件（检查点时不存在）",
    switchSession: "▸ 要切换到 \"{name}\"，请退出并运行：reasonix chat --session {name}",
    freshSession: "▸ 要开始新会话，请退出并运行：reasonix chat（不带 --session 标志）",
    modelSet: "▸ 模型：{id}",
    presetSet: "▸ 预设：{name} · {model}",
  },
};
